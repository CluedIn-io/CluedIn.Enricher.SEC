...
using Constants = CluedIn.ExternalSearch.Providers.SEC.Constants;

namespace CluedIn.Provider.SEC
{
    public class SECSearchProviderProvider : ProviderBase, IExtendedProviderMetadata, IExternalSearchProviderProvider
    {
        public IExternalSearchProvider ExternalSearchProvider { get; }

        public SECSearchProviderProvider([System.Diagnostics.CodeAnalysis.NotNull] ApplicationContext appContext) : base(appContext, GetMetaData())
        {
        ...
        public override async Task<CrawlJobData> GetCrawlJobData(ProviderUpdateContext context, IDictionary<string, object> configuration, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            ...
            var result = new SECExternalSearchJobData(configuration);
        ...
        {
            if (jobData is SECExternalSearchJobData result)
            {
                return await Task.FromResult(result.ToDictionary());
            }

            throw new InvalidOperationException($"Unexpected data type for {nameof(SECExternalSearchJobData)}, {jobData.GetType()}");
        }
